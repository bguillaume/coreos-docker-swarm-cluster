...
while [ "`curl -s http://${COREOS_PRIVATE_IPV4}:2379/v2/keys/nodes/bootstrapping | jq -r '.node.value'`" != "null" ]; do
  echo "An other node is bootstrapping, waiting 5 seconds..."
  sleep 5
done

# Set a lock
etcdctl set /nodes/bootstrapping ${COREOS_PUBLIC_IPV4} --ttl 180
...
$slack -m "_${COREOS_PUBLIC_IPV4}_: Bootstraping a *$1* node" -u $SLACK_WEBHOOK_URL -c "#$SLACK_CHANNEL"
...
do
   ...
   $slack -m "_${COREOS_PUBLIC_IPV4}_: Started unit _${unit}_ on ${COREOS_PUBLIC_IPV4}" -u $SLACK_WEBHOOK_URL -c "#$SLACK_CHANNEL"
done


$slack -m "_${COREOS_PUBLIC_IPV4}_: Bootstrap completed for *$1* node at ${COREOS_PUBLIC_IPV4}!" -u $SLACK_WEBHOOK_URL -c "#$SLACK_CHANNEL"
